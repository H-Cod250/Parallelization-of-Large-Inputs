...
// This loop will compare the total values between the days and returns the day with the most and least worked hours
...
// Calculate salary
#pragma omp parallel for num_threads(4) reduction(+:total_hours)
    for (int i = 0; i < days; i++) {
         int Current_Thread = omp_get_thread_num();  // Get the current thread number
         int Total_Threads = omp_get_num_threads(); // Get the total number of thread
         printf("Thread %d out of %d threads is processing day %d.\n", Current_Thread, Total_Threads, i);
